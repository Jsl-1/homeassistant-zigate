...
from .core.gateway import ZigateGateway
from .const import (DOMAIN,
                    DATA_ZIGATE, DATA_ZIGATE_ATTRS,
                    DATA_ZIGATE_DEVICES, DATA_ZIGATE_COMPONENTS,
                    DATA_ZIGATE_CONFIG, DATA_ZIGATE_GATEWAYS,
                    ADDR, IEEE, SCAN_INTERVAL,
                    GROUP_ENTITY_ID_FORMAT, GROUP_NAME_ALL_ZIGATE,
                    ZIGATE_BASE_COMPONENT_ID,                    
                    CONF_ZIGATE_CHANNEL, CONF_ZIGATE_GATEWAY_ID,
                    CONF_ZIGATE_ENABLE_LED, CONF_ZIGATE_GPIO,
                    CONF_ZIGATE_POLLING, SUPPORTED_PLATFORMS)
...
CONFIG_SCHEMA = vol.Schema({
    DOMAIN: vol.Schema({
        ...
        vol.Optional(CONF_ZIGATE_GATEWAY_ID, default='zigate'): cv.string,
    })
}, extra=vol.ALLOW_EXTRA)

async def async_setup(hass, config):
    """Setup zigate platform."""
    hass.data[DATA_ZIGATE] = {}
    hass.data[DATA_ZIGATE][DATA_ZIGATE_COMPONENTS] = {}
    hass.data[DATA_ZIGATE][DATA_ZIGATE_DEVICES] = {}
    hass.data[DATA_ZIGATE][DATA_ZIGATE_ATTRS] = {}
    hass.data[DATA_ZIGATE][DATA_ZIGATE_GATEWAYS] = {}

    if DOMAIN not in config:
        return True

    conf = config[DOMAIN]
    hass.data[DATA_ZIGATE][DATA_ZIGATE_CONFIG] = conf
    ...
    if not hass.config_entries.async_entries(DOMAIN):
        hass.async_create_task(hass.config_entries.flow.async_init(
            ...
            data={
                CONF_PORT: conf.get(CONF_PORT),
                CONF_HOST: conf.get(CONF_HOST),
                CONF_ZIGATE_GATEWAY_ID: conf.get(CONF_ZIGATE_GATEWAY_ID, 'zigate'),
                CONF_ZIGATE_GPIO: conf.get(CONF_ZIGATE_GPIO, False),
                CONF_ZIGATE_ENABLE_LED: conf.get(CONF_ZIGATE_ENABLE_LED, True),
                CONF_ZIGATE_POLLING: conf.get(CONF_ZIGATE_POLLING, True),
                CONF_ZIGATE_CHANNEL: conf.get(CONF_ZIGATE_CHANNEL),
            }
        ))
    return True
...
async def async_setup_entry(hass, config_entry):
    ...
    _LOGGER.debug('Polling : %s', polling)
    _LOGGER.debug('Component Id : %s', zigate_gateway_id)

    zigate_gateway = hass.data[DATA_ZIGATE][DATA_ZIGATE_GATEWAYS].get(zigate_gateway_id, None)
    if zigate_gateway is None:
        zigate_gateway = ZigateGateway(hass, config_entry)
        hass.data[DATA_ZIGATE][DATA_ZIGATE_GATEWAYS][zigate_gateway_id] = zigate_gateway
    
        device_registry = await \
        hass.helpers.device_registry.async_get_registry()
        device_registry.async_get_or_create(
            config_entry_id=config_entry.entry_id,
            connections={(CONNECTION_ZIGBEE, zigate_gateway_id)},
            identifiers={(DOMAIN, zigate_gateway_id)},
            name="Zigate Coordinator",
            manufacturer="Zigate",
            model=zigate_gateway.model,
        )

    # component = hass.data[DATA_ZIGATE][DATA_ZIGATE_COMPONENTS].get(zigate_gateway_id, None)
    # if component is None:
    #     component = hass.data[DATA_ZIGATE][DATA_ZIGATE_COMPONENTS][zigate_gateway_id] = EntityComponent(
    #         _LOGGER, DOMAIN, hass, SCAN_INTERVAL, GROUP_NAME_ALL_ZIGATE
    #     )  
    #     hass.data[DATA_ZIGATE][DATA_ZIGATE_COMPONENTS][zigate_gateway_id] = component
    ...
    # hass.data[DATA_ZIGATE][DATA_ZIGATE_DEVICES][zigate_gateway_id] = entity
